# Plan 6c.1: Hindsight Memory Provider Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:executing-plans.

**Goal:** Add Hindsight as an external memory provider. Cloud mode only (`https://api.hindsight.vectorize.io`) — local embedded mode requires the Python SDK and is out of scope. Exposes three tools: `hindsight_retain`, `hindsight_recall`, `hindsight_reflect`.

**Architecture:** Follow the pattern established in Plan 6c: a `Hindsight` struct in `tool/memory/memprovider/hindsight.go` implementing the `Provider` interface, backed by `httpJSON`. Three tool handlers. Factory entry so `memory.provider: hindsight` selects it.

**Tech Stack:** Go 1.25, existing `httpJSON`. No new deps.

---

## Task 1: Config block

- [ ] Add `HindsightConfig` to `config/config.go`:

```go
// HindsightConfig holds the Hindsight provider configuration.
...
Add `Hindsight HindsightConfig `yaml:"hindsight,omitempty"`` to `MemoryConfig`.

- [ ] `go test ./config/...` — PASS.
- [ ] Commit `feat(config): add HindsightConfig`.

---

## Task 2: Hindsight provider

- [ ] Create `tool/memory/memprovider/hindsight.go`:
...
func (h *Hindsight) retain(ctx context.Context, content, context_ string) error {
	url := h.cfg.BaseURL + "/banks/" + h.cfg.BankID + "/retain"
	body := map[string]any{"content": content, "context": context_}
...
- [ ] Create `tool/memory/memprovider/hindsight_test.go` exercising retain/recall/reflect against an `httptest.Server`.
- [ ] Run `go test ./tool/memory/memprovider/ -run TestHindsight` — PASS.
- [ ] Commit `feat(tool/memory): add Hindsight cloud memory provider`.

---

## Task 3: Factory entry

- [ ] Add case to `factory.go`:
...
- [ ] Run `go test ./tool/memory/memprovider/...` — PASS.
- [ ] Commit `feat(tool/memory): wire hindsight into memprovider factory`.
