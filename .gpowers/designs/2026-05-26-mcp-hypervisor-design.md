...
- 与 Node 共享的 `STORAGE_DIR/plugins/anythingllm_mcp_servers.json` 配置（双进程部署可热切换）
...
| 配置文件 | `STORAGE_DIR/plugins/anythingllm_mcp_servers.json`，结构 `{ mcpServers: { name: { command/args/env / url/headers / type, anythingllm: { autoStart, suppressedTools[] } } } }` |
...
| Tool suppression | `config.anythingllm.suppressedTools[]`，写回 JSON |
...
func buildMCPServerEnv(srv *ServerConfig) map[string]string {
    ...
    if runtime.GOOS != "windows" && os.Getenv("ANYTHING_LLM_RUNTIME") == "docker" {
...
读多写少。`hv.mu` 保护内存状态；JSON 文件 IO 用单独的 `cfgMu` 串行化（toggle-tool / delete / 写 anythingllm.suppressedTools 等）：

写入用 `os.WriteFile(tmp, ...) → os.Rename(tmp, final)` 原子替换。

### 4.6 工具压制名单

对齐 Node：写在 `mcpServers[name].anythingllm.suppressedTools[]`，listTools 时过滤。`ToolsAsPlugins()` 过滤后再产出 plugin。
...
func (h *Hypervisor) configFilePath() string {
    return filepath.Join(h.cfg.StorageDir, "plugins", "anythingllm_mcp_servers.json")
}
```
...
2. **`anythingllm.autoStart=false` 的服务什么时候启动**？  
...
6. **Per-server concurrency limit** — 默认每服务器 4 个并发调用，支持 `anythingllm.maxConcurrency` 覆盖；超限返回 `429 CONCURRENCY_LIMIT`，不排队、不阻塞。
