...
//
// Rules:
//   - gateway has its own dedicated panel that ignores subkey — return null.
...
//     map (or unspecified, which defaults to map). keyed_map/list shapes
//     require an element selection, so they are skipped.
//   - if nothing matches, return null (caller falls through to the
//     group-level empty/placeholder view).
